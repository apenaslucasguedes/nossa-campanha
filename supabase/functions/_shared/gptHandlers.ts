...
import { ApiError, GPT_KEY_HEADER, extractGptKey, gptRpcError, sha256Hex, validateGptDicePool, validateGptRollRequest, type GptDicePoolInput, type GptRollRequestInput } from './contracts.ts'
...
export async function resolveDicePoolRequest(request: Request, rawBody: unknown, rpc: RpcCaller): Promise<GptResult> {
  const rawKey=extractGptKey(request)
  if(!rawKey) return {ok:false,error:missingKeyError()}
  if(!validateGptDicePool(rawBody)) return {ok:false,error:new ApiError('INVALID_ACTION','Conjunto de dados, modificador ou motivo inválido.')}
  const input:GptDicePoolInput=rawBody
  const keyHash=await sha256Hex(rawKey)
  const {data,error}=await rpc('request_dice_pool_for_gpt',{lookup_key_hash:keyHash,payload:input})
  if(error) return {ok:false,error:gptRpcError(error)}
  return {ok:true,data,summary:'Dados solicitados; aguardando o jogador rolar no site.'}
}
