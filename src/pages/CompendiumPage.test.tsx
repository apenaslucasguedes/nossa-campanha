...
describe('Compêndio',()=>{
 it('renderiza seis classes, cinco atributos, condições e as nove regiões reais',()=>{const{container}=renderPage();expect(container.querySelectorAll('.class-compendium-card')).toHaveLength(6);const attributes=within(screen.getByRole('region',{name:'Atributos'}));for(const name of ['Força','Agilidade','Intelecto','Presença','Instinto'])expect(attributes.getByRole('heading',{name})).toBeInTheDocument();for(const name of ['Ferido','Exausto','Amedrontado','Envenenado','Imobilizado','Desorientado','Corrompido','Caído'])expect(screen.getByRole('heading',{name})).toBeInTheDocument();expect(container.querySelectorAll('.region-compendium-card')).toHaveLength(9)})
 ...
 it('oferece navegação desktop e móvel para a rota autenticada',()=>{render(<MemoryRouter><AppShell/></MemoryRouter>);const links=screen.getAllByRole('link',{name:'Compêndio'});expect(links).toHaveLength(2);for(const link of links)expect(link).toHaveAttribute('href','/compendio')})
})
