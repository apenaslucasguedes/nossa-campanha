...
function AttributeMeter({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <div className="attribute-meter" aria-label={`Valor de ${label}`}>{[0, 1, 2, 3].map((number) => <button key={number} type="button" className={number > 0 && number <= value ? 'is-filled' : ''} aria-pressed={value === number} onClick={() => onChange(number)}><span>{number}</span></button>)}</div>
}
