...
export function CharacterPortrait({ classKey, name, compact = false, avatar, artworkFit = 'contain' }: { classKey: ClassKey; name?: string; compact?: boolean; avatar?: AvatarOptions; artworkFit?: 'contain' | 'cover' }) {
  ...
  return (
    <div className={`character-portrait ${compact ? 'character-portrait--compact' : ''}`} data-character-class={classKey}>
      {hasLayerColors ? <EditableCharacterArtwork classKey={classKey} colors={layerColors!} fit={artworkFit} /> : <CharacterArtwork classKey={classKey} name={name} loading={compact ? 'lazy' : 'eager'} avatar={avatar} fit={artworkFit} />}
