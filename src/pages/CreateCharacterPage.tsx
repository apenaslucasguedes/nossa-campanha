...
function ClassStep({ selected, choose }: { selected: ClassKey; choose: (key: ClassKey) => void }) {
  return <><CreationStepHeader step={0} title="Escolha sua classe"><p>Compare função, atributo principal e recursos de nível 1. A seleção define a sugestão inicial de atributos.</p></CreationStepHeader><div className="class-grid">{CLASSES.map((item) => { const preview = calculateDerived(item.key, item.suggested); return <button type="button" className={item.key === selected ? 'class-option selected' : 'class-option'} data-character-class={item.key} onClick={() => choose(item.key)} key={item.key} aria-pressed={item.key === selected}><CharacterPortrait classKey={item.key} name={item.name} compact /><span className="class-option__selection">{item.key === selected ? 'Classe selecionada' : 'Selecionar classe'}</span><span className="class-option__role">{item.role}</span><strong className="class-option__name"><Icon name={classIcon(item.key)} size={22} decorative />{item.name}</strong><dl><div><dt>Atributo</dt><dd><Icon name={attributeIcon(item.primary)} size={16} decorative />{ATTRIBUTE_NAMES[item.primary]}</dd></div><div><dt>Recurso</dt><dd><Icon name="recurso-de-classe" size={16} decorative />{item.resource}</dd></div><div><dt>Vida</dt><dd><Icon name="vitalidade" size={16} decorative />{preview.vitality}</dd></div></dl><div className="class-option__abilities"><ClassAbility label="Básica" ability={item.basicAbility} /><ClassAbility label="Inicial" ability={item.initialAbility} /></div></button> })}</div></>
}
...
function AttributesStep({ value, onChange, derived, resourceLabel }: { value: Attributes; onChange: (value: Attributes) => void; derived: ReturnType<typeof calculateDerived>; resourceLabel: string }) {
  return <><CreationStepHeader step={1} title="Distribua seus atributos"><p>Use os valores <strong>3, 2, 1, 1 e 0</strong>. Clique nas barrinhas para ajustar sem abrir lista.</p></CreationStepHeader><div className="attribute-grid">{ATTRIBUTE_KEYS.map((key) => <div className="attribute-control" key={key}><AttributeItem attribute={key} value={value[key]} control={<AttributeMeter label={ATTRIBUTE_NAMES[key]} value={value[key]} onChange={(number) => onChange({ ...value, [key]: number })} />} /></div>)}</div>{!isValidAttributeDistribution(value) ? <ErrorBanner>A distribuição deve conter exatamente 3, 2, 1, 1 e 0.</ErrorBanner> : null}<Derived derived={derived} resourceLabel={resourceLabel} /></>
}

function AttributeMeter({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <div className="attribute-meter" role="group" aria-label={`Valor de ${label}: ${value}`}>{[0, 1, 2, 3].map((number) => <button key={number} type="button" className={number <= value ? 'is-filled' : ''} aria-label={`Definir ${label} como ${number}`} aria-pressed={value === number} onClick={() => onChange(number)}><span>{number}</span></button>)}</div>
}
