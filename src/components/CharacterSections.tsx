...
export function CharacterReview({ input, derived }: { input: { name: string; presentation: string; origin: string; appearance: string; personality: string; objective: string; fear: string; bond: string; classKey: ClassKey; attributes: Attributes; specialties: Specialty[]; avatar: AvatarOptions }; derived: { vitality: number; defense: number; inventoryCapacity: number; resource: number } }) {
  const role = getClassDefinition(input.classKey)
  return <div className="character-review"><CharacterHeader classKey={input.classKey} name={input.name} level={1} presentation={input.presentation} origin={input.origin} currentBond={input.bond} /><div className="character-review__body"><dl className="mechanic-readout"><div><dt><Icon name="vitalidade" size={18} decorative />Vitalidade</dt><dd>{derived.vitality}</dd></div><div><dt><Icon name="recurso-de-classe" size={18} decorative />{role.resource}</dt><dd>{derived.resource}</dd></div><div><dt><Icon name="defesa" size={18} decorative />Defesa</dt><dd>{derived.defense}</dd></div><div><dt><Icon name="inventario" size={18} decorative />Inventário</dt><dd>{derived.inventoryCapacity}</dd></div></dl><section><h3>Atributos</h3><div className="attribute-readout">{ATTRIBUTE_KEYS.map((key) => <AttributeItem key={key} attribute={key} value={input.attributes[key]} />)}</div></section><section><h3>Especialidades</h3><div className="specialty-review">{input.specialties.map((item) => <SpecialtyCard key={item} name={item} />)}</div></section><section className="ability-grid"><AbilityCard label="Habilidade básica" ability={role.basicAbility} /><AbilityCard label="Habilidade inicial" ability={role.initialAbility} /></section><NarrativeSection entries={[{ label: 'Aparência', value: input.appearance }, { label: 'Personalidade', value: input.personality }, { label: 'Objetivo', value: input.objective }, { label: 'Medo', value: input.fear }, { label: 'Vínculo', value: input.bond }]} /><p className="creation-confirmation"><Icon name="personagens" size={22} decorative /><span><strong>Esta confirmação cria o personagem real.</strong> A ficha será gravada na campanha somente ao acionar “Criar meu personagem”.</span></p></div></div>
}
