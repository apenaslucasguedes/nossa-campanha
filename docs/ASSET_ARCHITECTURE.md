...
1. `.auren-map__water` é uma única camada ampliada dentro de `.auren-map__stage` e recebe `agua.jpg` diretamente em `background-image`. A textura repetida usa escala ampla, baixa opacidade e filtros de contraste, saturação e luminosidade sobre a cor-base sólida `#102936`. Como pertence ao mesmo plano transformável, acompanha zoom e pan junto com o continente. Uma vinheta radial suave no viewport reduz a uniformidade nas extremidades sem adicionar outra cópia da textura.
