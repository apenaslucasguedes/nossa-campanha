...
export function CharacterCard({ character, compact = false }: { character: Character; compact?: boolean }) {
  ...
  return <article className="character-card"><div className="card-heading"><div><p className="eyebrow">Nível {character.level}</p><h2>{character.name}</h2></div><span className="class-mark">{getClassDefinition(character.class_key).name}</span></div>
