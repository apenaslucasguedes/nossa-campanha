...
/** @deprecated Use listMyCampaigns/getCampaignDashboard from './campaignsList' with an explicit campaignId. Kept only for the legacy-route redirect helper. */
export async function getMyCampaign(userId: string): Promise<CampaignDashboard | null> {
  const { data: membership, error: memberError } = await supabase.from('campaign_members').select('*').eq('user_id', userId).order('joined_at').limit(1).maybeSingle()
...
export async function moveCharacterToCampaign(characterId: string, campaignId: string) {
  const { error } = await supabase.rpc('assign_character_to_campaign', { target_character: characterId, target_campaign: campaignId })
  if (error) { if (error.message.includes('SEAT_TAKEN')) throw new Error('Você já possui um personagem nesta campanha.'); if (error.code === '42501') throw new Error('Você não tem permissão para esta ação.'); throw new Error('Não foi possível vincular o personagem à campanha.') }
}
...
export type CreateCharacterInput={name:string;class_key:ClassKey;presentation:string;origin:string;appearance:string;personality:string;objective:string;fear:string;bond:string;attributes:Attributes;specialties:Specialty[];avatar:AvatarOptions;campaign_id?:string|null}
