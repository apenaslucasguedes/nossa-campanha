...
export function AurenMap({locations=[],selected,onSelect}:{locations?:readonly MapLocation[];selected:AurenRegionId|null;onSelect:(id:AurenRegionId|null)=>void}){
  ...
  useEffect(()=>{const element=viewport.current;if(!element)return;const handleWheel=(event:globalThis.WheelEvent)=>{const next=zoomAurenMap(transform,transform.scale+(event.deltaY<0?.2:-.2),viewportSize);if(next===transform)return;event.preventDefault();event.stopPropagation();setTransform(next)};element.addEventListener('wheel',handleWheel,{passive:false});return()=>element.removeEventListener('wheel',handleWheel)},[transform,viewportSize])
  ...
  return <section className="auren-map" aria-label="Mapa interativo de Auren">
    ...
    <div ref={viewport} className="auren-map__viewport" data-testid="map-viewport" data-water-pattern="repeat" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={cancelDrag} onDragStartCapture={(event)=>event.preventDefault()}>
      ...
      <div className="auren-map__stage" data-testid="map-stage" data-map-scale={transform.scale} data-map-x={transform.x} data-map-y={transform.y} style={{width:AUREN_MAP_WIDTH,height:AUREN_MAP_HEIGHT,transform:`matrix(${matrix.a},0,0,${matrix.d},${matrix.e},${matrix.f})`}}>
        <div className="auren-map__water" data-map-layer="water" aria-hidden="true" style={{backgroundImage:`url("${mapRegistry.auren.water}")`}} />
