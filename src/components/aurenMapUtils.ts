...
export function prepareAurenSvg(source:string){
  ...
  for(const id of aurenNonInteractiveLayerIds){const layer=document.getElementById(id);if(!layer)continue;layer.setAttribute('aria-hidden','true');layer.setAttribute('data-non-interactive','true');layer.setAttribute('display','none')}
