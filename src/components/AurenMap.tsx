...
export function AurenMap({locations=[],selected,onSelect}:{locations?:readonly MapLocation[];selected:AurenRegionId|null;onSelect:(id:AurenRegionId|null)=>void}){
  ...
  useEffect(()=>{const element=viewport.current;if(!element)return;const handleWheel=(event:globalThis.WheelEvent)=>{event.preventDefault();event.stopPropagation();setTransform((value)=>zoomAurenMap(value,value.scale+(event.deltaY<0?.2:-.2),viewportSize))};element.addEventListener('wheel',handleWheel,{passive:false});return()=>element.removeEventListener('wheel',handleWheel)},[viewportSize])
