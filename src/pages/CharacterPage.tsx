...
export function CharacterPage() {
  ...
  return <div className="characters-page"><PageHeader eyebrow="Protagonistas" title="Personagens">Os dois assentos da campanha, seus recursos e condições atuais.</PageHeader>{error ? <ErrorBanner>{error}</ErrorBanner> : null}<section className="protagonist-grid" aria-label="Protagonistas da campanha">{[1, 2].map((seat) => { const member = data.members.find((item) => item.seat === seat); const character = data.characters.find((item) => item.owner_id === member?.user_id); return character ? <CharacterCard key={seat} character={character} playerName={readPlayerName(member)} action={<button className="card-action" type="button" onClick={() => openSheet(character.id)}>Abrir ficha</button>} /> : <EmptySeat key={seat} seat={seat} playerName={readPlayerName(member)} canCreate={member?.user_id === session?.user.id} /> })}</section>{selectedCharacter ? <section className="selected-sheet" id="ficha-selecionada" aria-label={`Ficha de ${selectedCharacter.name}`}>{saveError ? <ErrorBanner>{saveError}</ErrorBanner> : null}{editing && selectedCharacter.owner_id === session?.user.id ? <EditCharacter character={selectedCharacter} saving={saving} cancel={() => { setSaveError(''); setEditing(false) }} save={async (changes) => { try { setSaving(true); setSaveError(''); await updateMyCharacter(selectedCharacter.id, changes); await refresh(); setEditing(false) } catch (reason) { setSaveError(reason instanceof Error ? reason.message : 'Não foi possível salvar. O conteúdo preenchido foi preservado.') } finally { setSaving(false) } }} /> : <FilledSheet character={selectedCharacter} playerName={readPlayerName(selectedMember)} edit={selectedCharacter.owner_id === session?.user.id ? () => { if (window.confirm('Editar campos narrativos, vínculo atual e cores? Classe e atributos permanecerão protegidos.')) setEditing(true) } : undefined} />}</section> : <EmptyState title="Nenhum personagem criado" icon="personagens">A criação acontece em etapas e só grava dados depois da revisão. <Link to="/criar-personagem">Criar meu personagem</Link>.</EmptyState>}</div>
}

export function FilledSheet({ character, playerName, edit }: { character: Character; playerName?: string; edit?: () => void }) {
...
function EditCharacter({ character, saving, cancel, save }: { character: Character; saving: boolean; cancel: () => void; save: (changes: Pick<Character, 'presentation' | 'origin' | 'appearance' | 'personality' | 'objective' | 'fear' | 'current_bond' | 'avatar'>) => Promise<void> }) {
  ...
  return <form className="edit-sheet" onSubmit={(event) => { event.preventDefault(); void save({ ...form, avatar }) }}><header className="edit-sheet__header"><span><Icon name="diario" size={22} decorative /></span><div><h2>Editar narrativa e visual</h2><p>Classe, atributos e valores derivados não são alterados aqui.</p></div></header><div className="visual-builder edit-visual"><AvatarPreview avatar={avatar} name={character.name} /><div className="visual-controls"><label>Tom de pele<input type="color" value={avatar.skinTone} onChange={(event) => setAvatar({ ...avatar, skinTone: event.target.value })} /></label><label>Cabelo<input type="color" value={avatar.hair} onChange={(event) => setAvatar({ ...avatar, hair: event.target.value })} /></label><label>Roupa principal<input type="color" value={avatar.primaryColor} onChange={(event) => setAvatar({ ...avatar, primaryColor: event.target.value })} /></label><label>Roupa secundária<input type="color" value={avatar.secondaryColor} onChange={(event) => setAvatar({ ...avatar, secondaryColor: event.target.value })} /></label></div></div><div className="form-grid">{Object.entries(form).map(([key, value]) => <label className={key === 'presentation' || key === 'origin' ? '' : 'wide'} key={key}>{({ presentation: 'Gênero ou apresentação', origin: 'Origem', appearance: 'Aparência', personality: 'Personalidade', objective: 'Objetivo', fear: 'Medo', current_bond: 'Vínculo atual' } as Record<string, string>)[key]}{key === 'presentation' || key === 'origin' ? <input required value={value} onChange={(event) => set(key as keyof typeof form, event.target.value)} /> : <textarea required value={value} onChange={(event) => set(key as keyof typeof form, event.target.value)} />}</label>)}</div><div className="wizard-actions"><button type="button" disabled={saving} onClick={cancel}>Cancelar</button><button className="primary-button" disabled={saving}>{saving ? 'Salvando…' : 'Salvar alterações'}</button></div></form>
}
